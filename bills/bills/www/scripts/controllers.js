...
mainApp.controller('mainController', function ($scope, $http, $location) {
    ...
    function onDeviceReady() {
        ...
        if (device.platform == "Android") {
            var db = window.sqlitePlugin.openDatabase({ name: "costs.db" });

            db.transaction(createSchema, errorInSchema, successInSchema);
            function createSchema(tx) {
                tx.executeSql('CREATE TABLE IF NOT EXISTS passage (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, name TEXT NOT NULL)');
                tx.executeSql('CREATE TABLE IF NOT EXISTS pay (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, member_event_id INTEGER NOT NULL, amount REAL)');
                tx.executeSql('CREATE TABLE IF NOT EXISTS member_event_link (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE, member_id INTEGER NOT NULL, event_id INTEGER NOT NULL)');
                tx.executeSql('CREATE TABLE IF NOT EXISTS member (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,passage_id INTEGER NOT NULL, name TEXT NOT NULL)');
                tx.executeSql('CREATE TABLE IF NOT EXISTS event (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,passage_id INTEGER NOT NULL, name TEXT NOT NULL)');
                tx.executeSql('CREATE TABLE IF NOT EXISTS debt (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,passage_id INTEGER NOT NULL, who_member_id INTEGER NOT NULL, whom_member_id INTEGER NOT NULL, amount REAL NOT NULL)');
            }
            ...
            db.transaction(selectRecords, errorDB, successDB);
            function selectRecords(tx) {
                tx.executeSql('SELECT * FROM passage', [], successResults, errorInQuery);
            }
    ...
    $scope.viewPassage = function (id) {
        window.localStorage.setItem("passage_id", id);
        $location.path('passage_page');
    };
    ...
    $scope.addPassage = function () {
        ...
        function onDeviceReady() {
            ...
            if (device.platform == "Android") {
                ...
                var name_element = document.getElementById("passage_name");
                var name = name_element.value;

                db.transaction(insertRecord, errorDB, successDB);

                function insertRecord(tx) {
                    tx.executeSql('INSERT INTO passage (name) VALUES (?)', [name], SuccessInsert, errorInsert);
                }
                ...
                var db = window.openDatabase("costs.db", "1.0", "costs", 500000);
                db.transaction(function (transaction) {
                    var name = document.getElementById("passage_name").value;
...
mainApp.controller('passageController', function ($scope, $http, $location) {
    ...
    function onDeviceReady() {
        if (device.platform == "Android") {
            ...
            db.transaction(function (transaction) {
                var passage_id = window.localStorage.getItem("passage_id");
            ...
            db.transaction(function (transaction) {
                ...
                transaction.executeSql('SELECT debt.id, debt.amount, debt.who_member_id, debt.whom_member_id, m1.name as name1, m2.name as name2 FROM debt, member m1, member m2 WHERE m1.id = debt.who_member_id and m2.id = debt.whom_member_id and debt.passage_id =' + passage_id, [], function (tx, results) {
                    $scope.$apply(function () {
                        ...
                        $scope.custom_debts = m;

                        var members = $scope.members;
                        var debts = $scope.custom_debts;
...
mainApp.controller('eventController', function ($scope, $http, $location){
    ...
    function onDeviceReady() {
        if (device.platform == "Android") {
            var db = window.sqlitePlugin.openDatabase({ name: "costs.db" });
            db.transaction(function (transaction) {
                var event_id = window.localStorage.getItem("event_id");
    ...
    $scope.viewMember = function (id) {
        window.localStorage.setItem("member_event_id", id);
        $location.path('member_page');
    };
});
...
mainApp.controller('memberController', function ($scope, $http, $location) {
    ...
    function onDeviceReady() {
        if (device.platform == "Android") {
            var db = window.sqlitePlugin.openDatabase({ name: "costs.db" });
            db.transaction(function (transaction) {
                var member_event_id = window.localStorage.getItem("member_event_id");
